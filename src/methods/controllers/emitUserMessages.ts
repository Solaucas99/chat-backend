      ...
      if (conversations && conversations.length > 0) {
        ...
        const conversationMap = await Promise.all(
          conversations.map(async (conversation) => {
            ...
            // Getting all messages id's on this conversations
            const conversationsMessagesIds = conversation.get('messages');
            ...
            // Finding all messages of this conversation on Database
            const messages = await Promise.all(
              conversationsMessagesIds.map(async (convMessageId) => {
                const message = await MessagesRepository.readMessage({
                  messageid: convMessageId.toString(),
                });

                return {
                  messageid: message.get('messageid').toString(), //uuid
                  messageowner: message.get('messageowner').toString(),
                  messagedestination: message
                    .get('messagedestination')
                    .toString(), //uuid
                  messagetext: message.get('messagetext'),
                  datehour: new Date(message.get('datehour')),
                };
              }),
            );
